...
tuple<vector<int64_t>, vector<float>, vector<float>> GPUTimer::report_path(int ep_idx, int el, bool verbose) {
    ...
    int worst_ep_i;
    if (ep_idx == -1) {
        auto ep_slacks = torch::nan_to_num(slacks.index_select(0, timing_raw_db.endpoints_index), FLT_MAX);
        ...
        worst_ep_idx = timing_raw_db.endpoints_index[worst_ep].item<int>();
        worst_ep_i = torch::argmin(ep_slacks[worst_ep]).item<int>();
    } else {
        worst_ep_idx = ep_idx;
        worst_ep_i = torch::argmin(slacks[worst_ep_idx]).item<int>();
    }
    ...
    while (cur != -1) {
        ...
        if (prev != -1) {
            ...
            slack = slacks[cur][to_i].item<float>();
        }
        path.push_back(cur);
...
vector<vector<int64_t>> GPUTimer::report_K_path(int K, bool verbose) {
    ...
    auto [endpoints_index, tmp1] = torch::_unique(timing_raw_db.endpoints_index);
    auto ep_slacks = torch::nan_to_num(slacks.index_select(0, endpoints_index));
    ...
    endpoints_index = endpoints_index.contiguous();

    vector<vector<int64_t>> paths;

    for (int i = 0; i < K; i++) {
        auto [path, path_at, to_delay] = report_path(endpoints_index[indices[i]].item<int>(), false);
...
tuple<torch::Tensor, torch::Tensor> GPUTimer::report_criticality(int K, bool verbose, bool deterministic) {
    ...
    auto [from_pin_delay, pin_visited] = explore_path(
        at_prefix_pin, at_prefix_arc, at_prefix_attr, pinAt, arc_types, arcDelay, indices, ep_i_indices, endpoints_index, num_pins, K, deterministic);
